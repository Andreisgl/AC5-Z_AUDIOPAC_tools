...
number_of_parameters = 2 # Parameters for "project_data"
...
def init_project(): # Initializes project name and savefile stuff. MUST BE FIRST FUNCTION TO RUN!
    ...
    if IS_NEW_PROJECT:
        ...
        with open(SAVE_FOLDER + '/' + PROJECT_NAME + '/' + project_data_file, 'w') as project_meta:
            ...
            number_of_parameters = len(PROJECT_AUDIO_ARG_VALUES)
            answer_buffer = [] # Buffer order:
                                    # 0: Game
                                    # 1: Mode (RADIO or BGM)

            project_meta.write(str(number_of_parameters) + '\n')
            valid_input = False
            ...
            for i in range(number_of_parameters): # Add answers to "project_data" and "project_meta"
                project_data.append(PROJECT_AUDIO_ARG_VALUES[i])
        ...
        try:
            ...
            with open(SAVE_FOLDER + '/' + PROJECT_NAME + project_data_file):
                number_of_parameters = int(SFL.readline().strip('\n'))
...
def save_project(): # Saves current project.
    ...
    with open(SAVE_FOLDER + '/' + PROJECT_NAME + '/' + project_data_file, 'w') as PDF:
        for i in range(len(PROJECT_AUDIO_ARG_VALUES)):
            w = project_data[i] + '\n'
            PDF.write(w)
